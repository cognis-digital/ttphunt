"""Native cognis-connect emit for ttphunt — forward findings to any platform.

Maps ttphunt's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    ttphunt ... --format json | ttphunt-emit --to stix
    ttphunt-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out["title"] = rec.get("rule", {}).get("title", "")
        out["severity"] = rec.get("rule", {}).get("severity", "info")
        out["type"] = rec.get("rule", {}).get("technique", "")
        out["description"] = rec.get("rule", {}).get("description", "")
        out["tags"] = rec.get("rule", {}).get("tags", [])
